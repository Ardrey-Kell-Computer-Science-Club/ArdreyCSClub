      ...
      <section className="px-5 py-16 sm:px-8 sm:py-24 lg:px-12">
        <div className="mx-auto max-w-7xl">
          ...
          <Link href="/hackathons/ahahacks" className="group grid min-h-[31rem] border border-[var(--ak-orange)] bg-[var(--ak-surface)] md:grid-cols-[1.15fr_0.85fr]">
            <div className="flex flex-col p-6 sm:p-10 lg:p-14"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-[0.65rem] uppercase tracking-[0.14em] text-[var(--ak-orange)]"><span className="size-2 bg-[var(--ak-orange)]" /> Registration open</span><span className="text-[0.65rem] text-[var(--ak-dim)]">001</span></div><div className="mt-auto"><p className="mb-4 text-xs uppercase tracking-[0.16em] text-[var(--ak-muted)]">AKCS presents</p><h3 className="text-[clamp(3rem,8vw,7rem)] leading-[0.86] tracking-[-0.08em]">AHA<br /><span className="text-[var(--ak-orange)]">Hacks</span></h3><p className="mt-6 max-w-xl text-sm leading-7 text-[var(--ak-muted)]">The first AKCS hackathon: a beginner-friendly team sprint to build something useful, learn from mentors, and present what you made.</p></div></div>
            <div className="flex flex-col border-t border-[var(--ak-line)] bg-[var(--ak-bg)] p-6 md:border-l md:border-t-0 sm:p-10"><div className="grid gap-5 border-y border-[var(--ak-line)] py-6"><div><span className="text-[0.62rem] uppercase tracking-[0.14em] text-[var(--ak-dim)]">When</span><p className="mt-2 text-xl">Second week of September</p></div><div><span className="text-[0.62rem] uppercase tracking-[0.14em] text-[var(--ak-dim)]">Format</span><p className="mt-2 text-xl">Teams of 2–4</p></div><div><span className="text-[0.62rem] uppercase tracking-[0.14em] text-[var(--ak-dim)]">Experience</span><p className="mt-2 text-xl">Beginners welcome</p></div></div><div className="mt-auto flex items-center justify-between pt-8 text-xs uppercase tracking-[0.12em] text-[var(--ak-orange)]"><span>Open event details</span><ArrowRight className="size-5 transition-transform group-hover:translate-x-2" /></div></div>
          </Link>
        </div>
      </section>
